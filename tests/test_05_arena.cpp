...
#define print_arena_vals()                                                                                             \
    RANGE(i, 4)                                                                                                        \
    {                                                                                                                  \
        printf("| ");                                                                                                  \
        RANGE(j, 8)                                                                                                    \
        {                                                                                                              \
            printf(" %3ld" COLOR_RED " %2c " COLOR_RESET, i * 8 + j,                                                   \
                   (a.end - a.cap)[i * 8 + j] ? (a.end - a.cap)[i * 8 + j] : '.');                                     \
        }                                                                                                              \
        printf("\n");                                                                                                  \
    }

    TEST_CASE("Alloc: Non-aligned")
    {

        // Given a size, alignof is automatically decided,
        //   therefore, size is always a multiple of 2, unless packed
        struct __attribute__((packed)) Packed_3 {
            char x[3] = {'a', 'b', 'c'};
        };

        struct __attribute__((packed)) alignas(8) Aligned_8 {
            char x[3] = {'d', 'e', 'f'};
        };

        TEqualLong(sizeof(Packed_3), 3);
        TEqualLong(alignof(Packed_3), 1);

        TEqualLong(sizeof(Aligned_8), 8); // != 3
        TEqualLong(alignof(Aligned_8), 8);

        Arena a = perm;                      // 1 KB
        a.Make<char>(a.cap, DEFAULTS, '\0'); // Zeroed by default
        a = perm;                            // Reset to perm
        TEqualLong(a.Used(), 0);

        a.Make<Packed_3>(1, DEFAULTS);
        TEqualLong(a.Used(), 3);

        a.Make<Aligned_8>(1, DEFAULTS);
        TEqualLong(a.Used(), 3 + 5 + 8); // Padding!
